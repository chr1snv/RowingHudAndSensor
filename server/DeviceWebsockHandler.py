...
def handleWebSockClientDeviceRequests(client, datType, datStr):
	...
	if client.devId in Device.devices.keys():
		...
		if datType.startswith(b'status'):
			...
			client.send( device.devId, [('Stat', len(device.postStatus), device.postStatus), ('Time', len(timeStr), timeStr)] )
		elif datType.startswith(b'settings'):
			#setLen = str(len(device.postSettings)).encode('utf-8')
			print( 'req settings from dev' )
			device.send( Client.svrDevId, [('getSettings', 0, b'')] )
			print( 'sending last settings to browser' )
			lastSetTimeStr = str(device.lastSettingsTime).encode('utf-8')
			client.send( device.devId, [('Set', len(device.postSettings), device.postSettings), ('Time', len(lastSetTimeStr), lastSetTimeStr)] )
		elif datType.startswith(b'image'):
			#print( 'sending img to browser' )
			client.send( device.devId, [('Img', len(device.lastImage), device.lastImage)]  )
		else:
			cmd = datType.strip()
			val = datStr
