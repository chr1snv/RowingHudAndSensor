...
bool joinedWifiNetwork = false;
uint8_t joinedWifiChannel = 0;
...
uint8_t connectWiFi(uint8_t channelToCreateAp){ //returns channel set up on (for esp now config)
	uint8_t channelUsing = 0;
	...
	for( uint8_t num = 0; ((num < MAX_STORED_NETWORKS) && (!joinedWifiNetwork)); num++ ){
	...
	Serial.print( storedPassword ); Serial.println( "|" );
		for( uint8_t i = 0; ((i < numFoundNetworks) && (!joinedWifiNetwork)); ++i ){
          ...
          memcpy( APssid, storedNetwork,  SSIDPASSLEN);
					joinedWifiNetwork = true;
				}else{
	...
	if( !joinedWifiNetwork ){
	    ...
	    channelUsing = channelToCreateAp;
      queueStringForMorseLedOutput(APpassword, SSIDPASSLEN);
	}else{
		//connected to network so don't have to start an access point
		channelUsing = WiFi.channel();
	}
