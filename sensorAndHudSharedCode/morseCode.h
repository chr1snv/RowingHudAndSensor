...
unsigned long morseOutputIntervalMicros = 300000; //< 1/3rd of 1 second
...
uint16_t morseOutBuffIdx = 0; //index of caracter being displayed
char morseOutputBuffer[MORSE_OUT_BUFF_LEN]; //the buffer of characters looped over for display

uint16_t morseOutQueuedEndIdx = 0; //if buffer isn't full used to loop over a shorter distance
...
bool queueStringForMorseLedOutput(const char * str, uint8_t strLen){
    ...
    morseOutQueuedEndIdx = MORSE_OUT_BUFF_LEN-1; //string wraps so entire buffer is used
	}else{
    if( morseOutQueuedEndIdx < MORSE_OUT_BUFF_LEN-1 && firstWriteEndIdx > morseOutQueuedEndIdx )
      morseOutQueuedEndIdx = firstWriteEndIdx; //keep increasing until reaching full buffer size
  }
	//Serial.println("\nqSfmlo end");
	return retVal;
}
...
void morseOutputLedUpdate( uint8_t mOutputPin, bool invertOutput=false ){
	...
	if( outputSignalIdx >= mCharLen+3 ){ //past the end of the character signals + (intra character space)
		...
		outputSignalIdx = 0;
		morseOutBuffIdx += 1;
		if( morseOutBuffIdx >= MORSE_OUT_BUFF_LEN || morseOutBuffIdx > morseOutQueuedEndIdx+1)
			morseOutBuffIdx = 0; //end of characters to output wrap back to beginning
		c = morseOutputBuffer[morseOutBuffIdx];
		Serial.print("m");Serial.print(morseOutBuffIdx);Serial.print(":");
    Serial.print((uint8_t)c);Serial.print(":");Serial.println(morseIdxToAscii(c));
	}
